...
class CommentFormTests(CommentTestCase):
    def testInit(self):
        f = BogofilterCommentForm(Article.objects.get(pk=1))
    ...
    def testValidPost(self):
        ...
        f = BogofilterCommentForm(a, data=self.getValidData(a))
    ...
    def tamperWithForm(self, **kwargs):
        ...
        d.update(kwargs)
        f = BogofilterCommentForm(Article.objects.get(pk=1), data=d)
    ...
    def testProfanities(self):
        ...
        f = BogofilterCommentForm(a, data=dict(d, comment="What a rooster!"))
        self.assertFalse(f.is_valid())
