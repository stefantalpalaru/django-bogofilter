...
class FlagViewTests(CommentTestCase):
    ...
    def testFlagPost(self):
        ...
        c = BogofilterComment.objects.get(pk=pk)
...
def makeModerator(username):
    ...
    ct = ContentType.objects.get_for_model(BogofilterComment)
...
class ApproveViewTests(CommentTestCase):
    ...
    def testApprovePost(self):
        ...
        c = BogofilterComment.objects.get(pk=c1.pk)
...
class AdminActionsTests(CommentTestCase):
    ...
    def setUp(self):
        ...
        perms = Permission.objects.filter(
            content_type__app_label = 'bogofilter',
    ...
    def testActionsNonModerator(self):
        comments = self.createSomeComments()
        self.client.login(username="normaluser", password="normaluser")
        response = self.client.get("/admin/bogofilter/bogofiltercomment/")
    ...
    def testActionsDisabledDelete(self):
        ...
        comments = self.createSomeComments()
        self.client.login(username="normaluser", password="normaluser")
        response = self.client.get('/admin2/bogofilter/bogofiltercomment/')
        self.assertEqual(response.status_code, 200)
    ...
    def performActionAndCheckMessage(self, action, action_params, expected_message):
        response = self.client.post('/admin/bogofilter/bogofiltercomment/',
