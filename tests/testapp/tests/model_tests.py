...
class CommentManagerTests(CommentTestCase):
    ...
    def testInModeration(self):
        ...
        c1.save()
        c2.save()
        moderated_comments = list(BogofilterComment.objects.in_moderation().order_by("id"))
    ...
    def testRemovedCommentsNotInModeration(self):
        ...
        c1.save()
        c2.save()
        moderated_comments = list(BogofilterComment.objects.in_moderation())
        self.assertEqual(moderated_comments, [c1])

    def testForModel(self):
        c1, c2, c3, c4 = self.createSomeComments()
        article_comments = list(BogofilterComment.objects.for_model(Article).order_by("id"))
        author_comments = list(BogofilterComment.objects.for_model(Author.objects.get(pk=1)))
    ...
    def testPrefetchRelated(self):
        ...
        with self.assertNumQueries(3):
            qs = BogofilterComment.objects.prefetch_related('content_object')
